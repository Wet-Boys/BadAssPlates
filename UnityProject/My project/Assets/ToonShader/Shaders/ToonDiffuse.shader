    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float4, _Highlights)
                UNITY_DEFINE_INSTANCED_PROP( float4, _Shadows)
                ...
                UNITY_DEFINE_INSTANCED_PROP( fixed, _FresnelOn)
                UNITY_DEFINE_INSTANCED_PROP( float, _EmisiveMapIntensity)
                UNITY_DEFINE_INSTANCED_PROP( fixed, _EmisiveMapOn)
                UNITY_DEFINE_INSTANCED_PROP( float4, _EmisiveMapColor)
            UNITY_INSTANCING_BUFFER_END( Props )
            struct VertexInput {
                UNITY_VERTEX_INPUT_INSTANCE_ID
                float4 vertex : POSITION;
...
////// Emissive:
                ...
                float3 _FresnelOn_var = lerp( 0.0, (saturate((_FresnelColor_var.rgb*pow(1.0-max(0,dot(i.normalDir, viewDirection)),_FresnelExponent_var)))*_FresnelIntensity_var), UNITY_ACCESS_INSTANCED_PROP( Props, _FresnelOn ) );
                float4 _EmisiveMap_var = tex2D(_EmisiveMap,TRANSFORM_TEX(i.uv0, _EmisiveMap));
                float _EmisiveMapIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _EmisiveMapIntensity );
                float4 _EmisiveMapColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _EmisiveMapColor );
                float3 _EmisiveMapOn_var = lerp( 0.0, (_EmisiveMap_var.r*_EmisiveMapIntensity_var*_EmisiveMapColor_var.rgb), UNITY_ACCESS_INSTANCED_PROP( Props, _EmisiveMapOn ) );
                float3 emissive = ((UNITY_LIGHTMODEL_AMBIENT.rgb*_Diffuse_var.rgb)+_FresnelOn_var+_EmisiveMapOn_var);
                float4 _MainColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _MainColor );
                float4 _Shadows_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Shadows );
                float4 _Highlights_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Highlights );
                ...
                float3 finalColor = emissive + (_MainColor_var.rgb*(_Diffuse_var.rgb*lerp(_Shadows_var.rgb,_Highlights_var.rgb,dot((attenuation*_ToonRamp_var.rgb),float3(0.3,0.59,0.11))))*_LightColor0.rgb);
...
////// Lighting:
                ...
                float2 node_5309 = float2(node_7859,node_7859);
                float4 _ToonRamp_var = tex2D(_ToonRamp,TRANSFORM_TEX(node_5309, _ToonRamp));
                float3 finalColor = (_MainColor_var.rgb*(_Diffuse_var.rgb*lerp(_Shadows_var.rgb,_Highlights_var.rgb,dot((attenuation*_ToonRamp_var.rgb),float3(0.3,0.59,0.11))))*_LightColor0.rgb);
